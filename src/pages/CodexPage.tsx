import { zodiacMyths, mythSources } from '../data/knowledgeContext';
import { CodexGuide } from '../components/cosmic/CodexGuide';
...
export default function CodexPage() {
  ...
  useEffect(()=>{document.title = `${entityId ? entityById[entityId]?.name || 'Mục không tồn tại' : 'Cosmic Codex'} · Mystical Self`;},[entityId]);
...
function CodexIndex() {
  ...
  return <div className="container page cosmos codex-page"><Link className="back-link" to="/"><ArrowLeft size={16}/> Trở về vũ trụ</Link><div className="codex-title"><div><div className="eyebrow">MỤC LỤC CỦA BẦU TRỜI BIỂU TƯỢNG</div><h1>Cosmic <em>Codex.</em></h1><p>Chọn một dấu hiệu. Lần theo những đường nối.</p></div><span className="codex-counter"><strong>{knowledgeEntities.length}</strong> mục khám phá</span></div>
    ...
    </div><p className="cosmic-caption">Chiêm tinh là hệ diễn giải biểu tượng. Các mục thiên văn được phân biệt riêng. <a href={`${import.meta.env.BASE_URL}credits.txt`} target="_blank" rel="noreferrer">Nguồn dữ liệu & ghi công ↗</a></p>
  </div>;
}
  ...
  return <div className="container page cosmos codex-page"><Link className="back-link" to="/codex"><ArrowLeft size={16}/> Mục lục Cosmic Codex</Link><div className="codex-entry-heading"><div><div className="eyebrow">{categoryLabels[entity.type]} · {entity.aliases[0]}</div><h1>{entity.name}</h1><p>{entity.description}</p></div><div className="cosmic-tabs" role="group" aria-label="Cách xem mục Codex"><button aria-pressed={view === 'profile'} onClick={() => setView('profile')}>Hồ sơ trực quan</button><button aria-pressed={view === 'connections'} onClick={() => setView('connections')}>Bản đồ liên kết ({related.length})</button></div></div>
    ...
    {view === 'connections' ? <ConnectionMap id={entity.id}/> : <div className="codex-entry-layout"><div className="codex-visual">{entity.type === 'house' ? <HouseWheel selected={entity.visual.index || 0}/> : <EntityArtwork entity={entity}/>}<p className="cosmic-caption">{entity.visual.atlas ? 'Artwork diễn giải nghệ thuật' : 'Sơ đồ học khái niệm, không phải dữ liệu lá số cá nhân'}</p>{entity.type === 'zodiac' && <Constellation index={entity.visual.index || 0}/>}</div>
      <div className="codex-entry-content"><div className="codex-keywords">{entity.keywords.map(keyword => <span key={keyword}>{keyword}</span>)}</div>
        ...
        {entity.type === 'zodiac' && <details className="codex-deeper"><summary>Hình tượng & cách đọc liên hệ</summary><p>{entity.metadata.mythology}</p><figure className="codex-myth"><div className="eyebrow">TRUYỀN THỐNG VĂN HÓA · KHÔNG PHẢI THIÊN VĂN</div><strong>{zodiacMyths[entity.visual.index || 0][0]}</strong><figcaption>{zodiacMyths[entity.visual.index || 0][1]}</figcaption><a className="text-link" href={mythSources[zodiacMyths[entity.visual.index || 0][2]]} target="_blank" rel="noreferrer">Nguồn hình tượng ↗</a></figure><p>Chủ tinh truyền thống và hiện đại là hai hệ quy chiếu. Liên tưởng nhà–cung không thay thế việc tính nhà trong lá số.</p></details>}
    ...
    <ExploreNext id={entity.id}/><div className="codex-footer-links"><Link to="/codex">Tiếp tục trong Codex</Link><Link to={`/?entity=${entity.id}`}>Trở về Explorer ↗</Link><Link to="/explore">Thử một công cụ tự khám phá ↗</Link></div>
  </div>;
}
